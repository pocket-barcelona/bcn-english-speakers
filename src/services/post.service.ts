...
export async function getMergedPosts(includeHiddenPosts = false) {
  ...
  return filterAndSortPosts(
    [...directusCollection, ...internalCollection],
    includeHiddenPosts
  );
}
...
async function getTransformedHeadlessPosts(): Promise<UnifiedBlogPost[]> {
  // example: https://cms.pocketbarcelona.com/items/posts/primavera-sound-festival?fields[]=*&fields[]=author.id&fields[]=author.name&fields[]=author.avatar&fields[]=author.author_type
  const posts = await directus.request(
    readItems("posts", {
      fields: [
        "slug",
        "status",
        { author: ["id", "name", "avatar", "author_type"] },
        "title",
        "content",
        "snippet",
...
export function transformHeadlessPosts(
  posts: HeadlessPost[]
): UnifiedBlogPost[] {
  return posts.map((p) => {
    ...
    if (updatedDate && createdDate) {
      updatedAfterPublished =
        updatedDate.valueOf() - createdDate.valueOf() > oneDayInMs * 2; // 2 days
    }
    ...
    const mapped: { __source: string } & UnifiedBlogPost = {
      ...
      data: {
        ...
        updatedDate:
          p.date_updated && updatedAfterPublished
...
async function getHeadlessPostById(
  slug: string
): Promise<{ data: HeadlessPost }> {
...
): Promise<{ data: HeadlessPost | null }> {
